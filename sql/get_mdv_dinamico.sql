CREATE OR REPLACE FUNCTION public.get_mdv_dinamico(p_estrutura text, p_distrito_filtro text)
 RETURNS TABLE(ciclo character varying, label character varying, total_visitas bigint, total_dias numeric)
 LANGUAGE plpgsql
AS $function$
BEGIN
    IF p_estrutura = 'Brasil' THEN
        ...
        metas_agrupadas AS (
            SELECT 
                m.ciclo,
                m.cod_setor,
                SUM(COALESCE(m.dias_trabalhados, 20)) AS total_dias
        ...
        SELECT 
            ...
            SUM(m.total_dias)::numeric AS total_dias
